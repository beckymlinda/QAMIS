...
<aside
    ...
    class="fixed inset-y-0 left-0 z-50 flex h-screen flex-col bg-[#0f2744] text-white shadow-xl transition-all duration-300 ease-in-out lg:shadow-none"
>
    <div class="flex h-16 items-center gap-3 border-b border-white/10 px-4 lg:px-5">
        <a href="{{ auth()->user()->homeRoute() }}" class="flex min-w-0 flex-1 items-center gap-2" @click="sidebarOpen = false">
            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#8cc63f] text-sm font-bold text-[#0f2744]">H</span>
            <span x-show="!sidebarCollapsed" class="truncate text-lg font-bold tracking-wide text-white">HEQAMIS</span>
        </a>
        <button
            type="button"

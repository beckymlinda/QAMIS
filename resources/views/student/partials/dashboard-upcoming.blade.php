...
<section aria-labelledby="student-upcoming-heading">
    <div class="mb-5">
        <h2 id="student-upcoming-heading" class="text-xl font-bold text-[#0f2744] sm:text-[1.375rem]">Upcoming Classes</h2>
        <p class="mt-1 text-xs text-gray-500">Your next scheduled sessions</p>
    </div>
    ...
    <div class="space-y-4">
        @forelse($upcomingSlots as $slot)
            ...
            <article class="relative overflow-hidden rounded-2xl bg-white p-5 shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('student.lms.show', $slotOffering) }}" class="inline-flex items-center gap-2 rounded-xl bg-[#0f2744] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1a3a5c] focus:outline-none focus:ring-2 focus:ring-[#8cc63f]">
        ...
        @empty
            <div class="rounded-2xl bg-white p-10 text-center shadow-md">
                ...
                <a href="{{ route('student.timetable') }}" class="mt-4 inline-flex text-sm font-semibold text-[#0f2744] hover:text-[#8cc63f]">View full timetable →</a>
            </div>
        @endforelse
    </div>
</section>

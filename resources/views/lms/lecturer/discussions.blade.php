<x-app-layout>
    ...
    <div class="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8" x-data="{ createOpen: false, editDiscussion: null }">
        ...
        <div class="flex justify-end">
            <button type="button" @click="createOpen = true" class="inline-flex items-center gap-2 rounded-xl bg-[#0f2744] px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#1a3a5c]">
                <i class="bi bi-plus-lg"></i> Add discussion
            </button>
        </div>
                        ...
                        <tr class="hover:bg-gray-50/50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('lecturer.lms.discussions.show', [$offering, $discussion]) }}" class="inline-flex items-center gap-1 rounded-lg bg-gray-50 px-3 py-1.5 text-xs font-semibold text-[#0f2744] ring-1 ring-gray-200 hover:bg-gray-100">Open chat</a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center text-gray-500">Nothing added yet. Click <strong>Add discussion</strong> to begin.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-show="createOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
                <h3 class="text-lg font-bold text-[#0f2744]">Add discussion topic</h3>
                <form method="POST" action="{{ route('lecturer.lms.discussions.store', $offering) }}" class="mt-4 space-y-4">
                    @csrf
        ...
        <div x-show="editDiscussion" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
                <h3 class="text-lg font-bold text-[#0f2744]">Edit discussion topic</h3>
                <form x-bind:action="editDiscussion ? '{{ url('lecturer/offerings/'.$offering->id.'/lms/discussions') }}/' + editDiscussion.id : '#'" method="POST" class="mt-4 space-y-4">
                    @csrf @method('PUT')
                    <input type="text" name="title" x-model="editDiscussion.title" required class="w-full rounded-xl border-gray-300 shadow-sm">
                    <textarea name="body" x-model="editDiscussion.body" rows="4" required class="w-full rounded-xl border-gray-300 shadow-sm"></textarea>
                    <label class="inline-flex items-center gap-2 text-sm"><input type="checkbox" name="is_pinned" value="1" x-bind:checked="editDiscussion?.is_pinned"> Pin topic</label>
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="editDiscussion = null" class="rounded-xl px-4 py-2 text-sm text-gray-600 hover:bg-gray-100">Cancel</button>

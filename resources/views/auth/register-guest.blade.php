<x-guest-layout>
    <x-slot name="title">Guest institution</x-slot>
    <x-slot name="heading">Register as guest institution</x-slot>
    <x-slot name="subheading">Presentation demo access — programmes and assessment tools only.</x-slot>
    ...
    <form method="POST" action="{{ route('register.guest') }}">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="password" class="block mt-1 w-full border-gray-300 focus:border-heqamis-green focus:ring-heqamis-green" type="password" name="password" required autocomplete="new-password" />
            <x-input-error :messages="$errors->get('password')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="password_confirmation" :value="__('Confirm Password')" class="text-heqamis-blue" />
            <x-text-input id="password_confirmation" class="block mt-1 w-full border-gray-300 focus:border-heqamis-green focus:ring-heqamis-green" type="password" name="password_confirmation" required autocomplete="new-password" />
